...
async def cars_cost_to_find(message: Message, state: FSMContext):
    ...
    await message.answer('Який ваш комфортний бюджет для купівлі авто?',
                         reply_markup=car_to_find_keyboard.cars_to_find_price_kb)
...
async def cars_year_to_find(message: Message, state: FSMContext):
    ...
    await message.answer('Автомобіль яких років випуску ви розглядаєте?',
                         reply_markup=car_to_find_keyboard.cars_to_find_year_kb)
...
async def cars_year_to_find(message: Message, state: FSMContext):
    ...
    await message.answer('Щоб отримати детальнішу інформацію, натисніть Отримати детальнішу інформацію',
                         reply_markup=start_keyboard.detail_info_and_main_kb)
