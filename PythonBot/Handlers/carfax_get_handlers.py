...
async def cars_cost_in_stock(message: Message, state: FSMContext):
    ...
    await message.answer("Щоб отримати звіт по авто, <b> залиште нижче VIN код або посилання на лот на аукціоні</b> та підтвердіть отриання Carfax", reply_markup=start_keyboard.back_bome_kb)
